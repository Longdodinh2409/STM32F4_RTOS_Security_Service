...
/* Buffer size */
#define SIZE_BUFFER		(SSD1306_WIDTH * SSD1306_HEIGHT / 8)
/* SSD1306 data buffer */
static uint8_t SSD1306_Buffer[SIZE_BUFFER];
static uint8_t SSD1306_SaveBuffer[SIZE_BUFFER];
...
void SSD1306_ScrollRight(uint8_t start_row, uint8_t end_row)
{
  ...
  SSD1306_WRITECOMMAND(0x29);  // send 0x26
  SSD1306_WRITECOMMAND(0x00);  // send dummy
  SSD1306_WRITECOMMAND(start_row);  // start page address
  SSD1306_WRITECOMMAND(0x02);  // time interval 64 frames
...
void SSD1306_ScrollLeft(uint8_t start_row, uint8_t end_row)
{
	SSD1306_WRITECOMMAND(SSD1306_DEACTIVATE_SCROLL);
	 ...
	 SSD1306_WRITECOMMAND(0x2A);  // send 0x26
	  SSD1306_WRITECOMMAND(0x00);  // send dummy
	  SSD1306_WRITECOMMAND(start_row);  // start page address
	  SSD1306_WRITECOMMAND(0x02);  // time interval 64 frames
	  SSD1306_WRITECOMMAND(end_row);  // end page address
...
void SSD1306_SoftwareScrollRight(uint32_t speed_ms)
{
    for (int p = 0; p < 8; p++)
    {
        uint8_t last_byte = SSD1306_Buffer[(p * 128) + 127];

        for (int c = 127; c > 0; c--)
        {
            SSD1306_Buffer[(p * 128) + c] = SSD1306_Buffer[(p * 128) + c - 1];
        }

        SSD1306_Buffer[p * 128] = last_byte;
    }
...
void SSD1306_SavePrevBitmapAnimation(void) {
	memcpy(SSD1306_SaveBuffer, SSD1306_Buffer, SIZE_BUFFER);
}

void SSD1306_RewriteBitmapOfPreviousStep(uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
	uint16_t x_end = x + w;
	uint16_t y_end = y + h;
	...
	for (int py = y; py < y_end; py++)
	{
		...
		{
			uint16_t page = py / 8;	// each 8px on Y axis is a page
			uint16_t bit = py % 8;
			uint16_t byte_index = px + (page * SSD1306_WIDTH);

			uint16_t bit_value = (SSD1306_SaveBuffer[byte_index] >> bit) & 0x01;

			if (bit_value == 1)
			{
				SSD1306_Buffer[byte_index] |= (1 << bit);
			}
			else
			{
				SSD1306_Buffer[byte_index] &= ~(1 << bit);
